...
/**
 * TDS server {@link Writer}
 */
public class TdsTaskWriter implements Writer<Result> {
     ...
     */
    private final TdsTaskWriteOperation writeOperation;
    ...
    public void write(Object datum) throws IOException {
        ...
        taskObj.put("type", writeOperation.getSink().getCampaignType()); //$NON-NLS-1$
        taskObj.put("record", recordObj); //$NON-NLS-1$
        taskObjs.add(taskObj);

        String resourceToCreate = "api/v1/campaigns/owned/" + writeOperation.getSink().getCampaignName() + "/tasks"; //$NON-NLS-1$ //$NON-NLS-2$
